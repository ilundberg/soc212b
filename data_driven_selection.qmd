...
### When the goal of the model is to predict for individuals
...
rsquared_individual <- individual_prediction |>
  summarize(rsquared = 1 - var(salary - yhat) / var(salary)) |>
  pull(rsquared)
...
While one might have hoped to tell a story about high-quality prediction, the dominant story in the individual-level prediction plot is one of poor prediction: players' salaries vary widely around the estimated regression line. To put that fact to a number, one might consider $R^2_\text{Individual}$ which involves a ratio of the variance of prediction errors to the total variance of the outcome.

$$
R^2_\text{Individual} = 1 - \frac{\text{V}\left(Y - \hat{Y}\right)}{\text{V}(Y)} = `r format(round(rsquared_individual,3),nsmall=3)`
$$
In other words, subtracting the predicted values from the individual players' salaries only reduces the variance by `r label_percent(accuracy = .1)(rsquared_individual)`. If the goal is to predict for individuals, the model does not seem very good!

### When the goal of the model is to estimate for subgroups
...
The social scientist might argue that the model is quite good for team salaries. If we take the goal to be to estimate the team average salary, then we might create an analogous version of $R^2$ where the unit of analysis is a team.

$$
R^2_\text{Group} = 1 - \frac{\text{V}\left(\hat{Y}_\text{Team} - \bar{Y}_\text{Team}\right)}{\text{V}(\bar{Y}_\text{Team})} = `r format(round(rsquared_team,3),nsmall=3)`
$$
By that metric, the model seems quite good, predicting away `r label_percent(accuracy = .1)(rsquared_team)` of the team-level variance in salary. Surprisingly, a model that was not very good at predicting for individuals might be quite good at predicting the team-average outcomes!
...
## How predictive performance metrics can choose the best estimator
...
$$
In our baseball example, the algorithm $\hat{f}_1$ makes an error when Mookie Betts has a salary of $Y_\text{MookieBetts} = $`r `baseball |> filter(player == "Betts, Mookie") |> pull(salary) |> label_currency(scale = 1e-6, suffix = "m", accuracy = .1)()` but the algorithm only predicts $\hat{f}_1(\vec{X}_\text{MookieBetts}) = $`r baseball |> mutate(yhat = predict(linear_regression, newdata = baseball)) |> filter(player == "Betts, Mookie") |> pull(yhat) |> label_currency(scale = 1e-6, suffix = "m", accuracy = .1)()`. The expected squared prediction error is the squared difference between these two values, taken on average over all players.
...
## Out-of-sample error: Illustration with k-nearest neighbors
...
estimates |>
  ...
  ggplot(aes(x = n_neighbors, y = value, color = name)) +
  geom_smooth(se = F) +
  theme_minimal() +
...
### Sample splitting for parameter tuning
...
```
For each $\lamdba$ value, the predictions are a column of `predicted`. The code below wrangles these predictions into a tidy format.
```{r}
...
2) Start a loop at $i$ = 1
3) Use fold $i$ as the test set and the other $k-1$ folds as the train set
4) Increment $i$ and repeat step 3 until all folds have been used as the test set in turn
5) Average the performance metric over the $k$ iterations
6) Optionally, repeat steps 1--5 many times for various random partitions of the data and average to reduce stochastic error
...
## Choosing among many algorithms
