...
This page contains embedded R code. If you are a Stata user, you can download do files that illustrate [Ordinary Least Squares](assets/stata_code/ols.do), [multilevel models](assets/stata_code/multilevel.do), [ridge regression](assets/stata_code/ridge.do), [LASSO regression](assets/stata_code/lasso.do), and [trees and forests](assets/stata_code/trees.do).
...
Here is a [PDF of this page](assets/pdf_chapters/algorithms_for_prediction.pdf) for those taking notes by hand.
...
baseball_population <- read_csv("data/baseball.csv") |>
  ...
  left_join(
    read_csv("data/baseball_3yrs.csv") |>
      filter(year == 2022) |>
      group_by(team) |>
      summarize(team_past_salary = mean(salary)),
    by = join_by(team)
  ) |>
  select(player, salary, position, team, team_past_record, team_past_salary) |>
